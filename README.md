...
**VR SNS で撮影した大量の写真の中から、「無駄な写真」をAIが自動で判定・整理してくれるツールです。**

VRChat などの写真フォルダを設定して起動するだけ。あとは **バックグラウンドで常駐して全自動**。
ゲーム内でシャッターを切るたびにAIが画像を分析し、「真っ暗」「UIの誤爆」「見切れた中途半端な写真」を自動で判定。NG（無駄写真）と判定されたら **即座に Windows 通知** でお知らせします。
...
- 📸 **撮ったそばから自動チェック** — バックグラウンドで常駐し、VR SNS でシャッターを切るたびにリアルタイムで判定が走ります。一度起動したら放置で OK
- 🔔 **リアルタイム・アラート通知** — NG と判定された画像は即座に Windows 通知でお知らせ。VR プレイ中でも無駄写真に気づける
- 🌐 **理由つきダッシュボード** — ブラウザで判定済み画像を一覧表示。「なぜ NG か」の AI コメントをそれぞれ確認できる
- 🤖 **完全ローカル AI 審査** — [LM Studio](https://lmstudio.ai/) + ビジョン対応モデル（Qwen3-VL など）で画像を分析。外部サーバー不使用
- ✏️ **ルールはカスタマイズ自由** — `rules.md` を編集するだけで判定基準を変更できます。再起動も不要
  - アバターがしっかり写っているか（OK）
  - 見栄えの良い風景写真か（OK）
  - 構図が中途半端で見切れていないか（NG）
  - UIやメニュー画面の誤爆ではないか（NG）
...
[4] LM Studio をインストール
        │
        ▼
[5] LM Studio で Qwen3-VL-4B をダウンロード
        │
        ▼
...
Qwen3 を使用する場合、デフォルトのコンテキスト長（`4096`）では画像処理に不足するため、モデルロード時または「Model Settings」から **Context Length を `8192` 以上** に変更することを推奨する。
