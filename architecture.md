...
VRChatのスクリーンショットを、ローカルVLM（LM Studio経由のVision Language Model）を用いてリアルタイムに判定し、無駄写真と良い写真を自動仕分けするポータブル・システム。

1. コンセプト
「見切れた失敗写真」や「UIの誤爆スクショ」等の無駄写真を、クラウドを介さず自分のPC内だけで完結して整理・判定する。ルールファイルを外部化し、柔軟に「自分にとっての良い写真」の基準を設定できる安定した環境を提供する。
...
4.2 判定ルールファイル (rules.md)
ユーザーが「何を無駄写真として弾くか」を柔軟に記述するファイル。

- **OK基準の例**: アバターがしっかり写っている写真、見栄えの良い風景写真。
- **NG基準の例**: 真っ暗な写真、UIメニューの誤爆スクショ、見切れたりブレたりしている失敗写真。
AIエンジンは毎回の写真ごとにこのファイルを読み込み、ルールにしたがって主観的にOK/NGを判定する。
